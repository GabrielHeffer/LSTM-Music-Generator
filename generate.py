...
def prepare_sequences(notes, pitchnames, n_vocab):
    """ Preparo das sequências usadas pela Rede Neural """
    # mapear entre notas e inteiros
...
def generate_notes(model, network_input_notes, network_input_offsets, network_input_durations, notenames, offsetnames,
                   durationames, n_vocab_notes, n_vocab_offsets, n_vocab_durations):
    ...
    for note_index in range(700):
        ...
        prediction = model.predict([note_prediction_input, offset_prediction_input, duration_prediction_input],
                                   verbose=0)
        ...
        print("Next note: " + str(int_to_note[predictedNote]) + " - Duration: " + str(
            int_to_duration[duration]) + " - Offset: " + str(int_to_offset[offset]))
...
def create_midi(prediction_output_all):
    """ converter a saída da previsão em notas e criar um arquivo midi das notas """
    offset = 0
    output_notes = []
    ...
    midi_stream.write('midi', fp='test_output_7.mid')


if __name__ == "__main__":
    # carregar as notas usadas pelo modelo
    with open('notes', 'rb') as filepath:
        notes = pickle.load(filepath)

    with open('duration', 'rb') as filepath:
    ...
    model = create_network(normalized_input_notes, n_vocab_notes, normalized_input_offsets, n_vocab_offsets,
                           normalized_input_durations, n_vocab_durations)

    prediction_output = generate_notes(model, network_input_notes, network_input_offsets,
                                       network_input_durations, notenames, offsetnames,
                                       durationames, n_vocab_notes, n_vocab_offsets, n_vocab_durations)
